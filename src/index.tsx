...
import type { StepCountData, Spec } from './NativeBackgroundStepCounter';
import { eventName, VERSION, NAME } from './NativeBackgroundStepCounter';

/* A way to check if the module is linked. */
const LINKING_ERROR =
  "The package 'react-native-background-step-counter' doesn't seem to be linked. Make sure: \n\n" +
  Platform.select({
  ...
  'https://github.com/Ugur-Atakan/react-native-background-step-counter`';
 ...
 */
const BackgroundStepCounterModule = isTurboModuleEnabled
  ? require('./NativeBackgroundStepCounter').default
  : NativeModules.BackgroundStepCounter;
...
export function startBackgroundService() {
  return BackgroundStepCounter.startBackgroundService();
}

export function stopBackgroundService() {
  return BackgroundStepCounter.stopBackgroundService();
}

    export function isStepCountingSupported(): Promise<Record<string, boolean>> {
      return BackgroundStepCounter.isStepCountingSupported();
    }
    ...
    export function startStepCounterUpdate(
      ...
      if (!BackgroundStepCounter.startStepCounterUpdate) {
      ...
      */
      ...
      BackgroundStepCounter.startStepCounterUpdate(from);
    ...
    export function stopStepCounterUpdate(): void {
      ...
      BackgroundStepCounter.stopStepCounterUpdate();
    }
